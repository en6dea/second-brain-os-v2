...
export async function перенестиСчета(состояние: Запись): Promise<{
  ...
  // Прежнее приложение хранило счёт на операции строкой с именем, а сам
  // счёт в справочник мог не попасть. Такие счета восстанавливаются по
  // именам из операций — иначе полторы тысячи записей остались бы без счёта.
  const восстановленные: Счёт[] = []
  const именаИзОпераций = new Set<string>()
  for (const операция of массив(состояние, 'operations')) {
    const имя = текст(операция.account)
    if (имя) именаИзОпераций.add(имя)
  }

  for (const имя of именаИзОпераций) {
    if (поИмени.has(имя.toLowerCase())) continue
    const запись = новаяЗапись({
      название: имя,
      тип: 'карта' as const,
      валюта: 'RUB' as const,
      // Остаток неизвестен: подтверждать его нечем. Ноль был бы обманом.
      фактическийОстаток: null,
      подтверждён: null,
      кредитныйЛимит: null,
      учитыватьВДоступных: true,
      архив: false,
      порядок: новые.length + восстановленные.length,
      legacy: { восстановленПоОперациям: true },
    }) as Счёт
    восстановленные.push(запись)
    поИмени.set(имя.toLowerCase(), запись.id)
  }

  const всеСчета = [...новые, ...восстановленные]
  if (всеСчета.length > 0) await база.accounts.bulkPut(всеСчета)

  return {
    строка: {
      название: 'Счета',
      перенесено: всеСчета.length,
      из: старые.length,
      пояснение:
        восстановленные.length > 0
          ? `${восстановленные.length} восстановлено по именам из операций, остаток не заполнен`
          : undefined,
    },
    поСтаромуId,
    поИмени,
...
export async function перенестиОперации(
  ...
  let безДаты = 0
  let переводовБезПолучателя = 0
  let переводовНеМеждуСчетами = 0
  ...
  for (const строка of старые) {
    ...
    // Корректировка баланса записывалась переводом. У неё свой тип, и знак
    // берётся из величины поправки.
    const поправка = числоИлиNull(строка.balanceAdjustment)
    const исходныйТип = ТИПЫ_ОПЕРАЦИЙ[текст(строка.type)] ?? 'расход'
    const тип: ТипОперации =
      исходныйТип === 'перевод' && поправка !== null ? 'корректировка' : исходныйТип

    if (тип === 'перевод') {
      переводовБезПолучателя += 1
      if (!названиеКатегории.toLowerCase().includes('между своими')) {
        переводовНеМеждуСчетами += 1
      }
    }
    ...
    новые.push(
      новаяЗапись({
        ...
        сумма: Math.abs(
          (тип === 'корректировка'
            ? деньгиИзСтарого(строка.balanceAdjustment)
            : деньгиИзСтарого(строка.amount)) ?? 0,
        ),
        счётId: тип === 'корректировка' && (поправка ?? 0) > 0 ? null : счётId,
        // У корректировки знак задаётся тем, на какой стороне стоит счёт.
        счётПолучательId:
          тип === 'корректировка' && (поправка ?? 0) > 0 ? счётId : null,
        категорияId:
  ...
  if (переводовБезПолучателя > 0) {
    предупреждения.push(
      `Переводов без счёта-получателя: ${переводовБезПолучателя}. В прежней версии второй счёт не записывался, поэтому такие переводы не двигают остатки. Укажите получателя в разделе «Операции».`,
    )
  }
  if (переводовНеМеждуСчетами > 0) {
    предупреждения.push(
      `Из них ${переводовНеМеждуСчетами} помечены переводом, но категория у них не «Между своими счетами». Тип не менялся: переклассифицировать ваши операции без подтверждения было бы неправильно. Проверьте их и при необходимости смените тип.`,
    )
  }
