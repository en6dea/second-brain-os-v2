...
/**
 * Время создания и изменения берётся из прежней записи, а не ставится «сейчас».
 *
 * Это не мелочь. Расчётный остаток счёта считается как подтверждённый остаток
 * плюс операции, записанные ПОСЛЕ подтверждения. Если всем перенесённым
 * операциям поставить сегодняшнее время, то к подтверждённому остатку
 * прибавится вся история сразу и остаток станет бессмысленным.
 */
export function времяЗаписи(строка: Запись): {
  createdAt?: string
  updatedAt?: string
} {
  const годится = (значение: string) => /^\d{4}-\d{2}-\d{2}T/.test(значение)
  const создано = текст(строка.createdAt)
  const изменено = текст(строка.updatedAt)
  return {
    ...(годится(создано) ? { createdAt: создано } : {}),
    ...(годится(изменено) ? { updatedAt: изменено } : {}),
  }
}
