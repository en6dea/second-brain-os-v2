...
export function ImportPage() {
  ...
  const статистика = useMemo(() => {
    ...
    const переводов = строки.filter((с) => с.перевод).length
    const обычные = строки.filter((с) => !с.перевод)
    const доходов = обычные.filter((с) => с.сумма > 0).length
  ...
  async function импортировать() {
    ...
    if (кЗаписи.length === 0 || !справочники) return

    const категорииПоId = new Map(
      справочники.категории.map((категория) => [категория.id, категория]),
    )
    const записи = кЗаписи.map((строка) => {
      const тип = строка.перевод
        ? 'перевод'
        : строка.сумма >= 0
          ? 'доход'
          : 'расход'
      const категория = строка.категорияId
        ? категорииПоId.get(строка.категорияId)
        : undefined
      return новаяЗапись({
        дата: строка.дата,
        тип,
        сумма: Math.abs(строка.сумма),
        // У перевода сторона важна: выбранный счёт — источник, если деньги
        // ушли, и получатель, если пришли. Вторая сторона из выписки
        // неизвестна и остаётся пустой, чтобы её проставил человек.
        счётId: строка.перевод && строка.сумма > 0 ? null : счётId || null,
        счётПолучательId:
          строка.перевод && строка.сумма > 0 ? счётId || null : null,
        категорияId: строка.перевод ? null : строка.категорияId || null,
        классРасхода:
          тип === 'расход' ? (категория?.классПоУмолчанию ?? null) : null,
            ...
            <div className="border-t border-line px-5 py-4">
              <div className="mb-4 rounded-3 border border-line bg-sunken p-4">
                ...
                <p className="mt-1 text-meta leading-relaxed text-ink-2">
                  ...
                  {статистика.переводов > 0
                    ? `, переводов между своими счетами: ${статистика.переводов}`
                    : ''}
                  .{' '}
                  {разбор.знакНадёжен
