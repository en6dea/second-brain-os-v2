...
import { следующаяДатаДохода } from '@/features/finance/model/IncomeSchedule'
...
export function FinanceOverview() {
  ...
  const посчитано = useMemo(() => {
    ...
    const датаДоходаПоГрафику = следующаяДатаДохода({
      сегодня: сегодня(),
      дниМесяца: данные.настройки.денежныйГоризонт?.дниДохода ?? [],
      переносВыходного:
        данные.настройки.денежныйГоризонт?.переносВыходного ?? 'без переноса',
    })
    const датаДохода =
      датаДоходаПоГрафику ??
      данные.настройки.денежныйГоризонт?.датаСледующегоДохода ??
      null
