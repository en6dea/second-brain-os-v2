...
import { рассчитатьДоДохода } from '@/features/finance/model/SafeToSpend'
import {
  ключПодтверждённогоДохода,
  следующаяДатаДохода,
  следующееСобытиеДохода,
} from '@/features/finance/model/IncomeSchedule'
import { MoneyRunwayCard } from '@/features/finance/MoneyRunwayCard'
...
export function FinanceOverview() {
  ...
  const данные = useLiveQuery(async () => {
    const [счета, операции, обязательства, категории, настройки, платежи] =
      await Promise.all([
    ...
    return { счета, операции, обязательства, категории, настройки, платежи }
  }, [])

  const посчитано = useMemo(() => {
    if (!данные) return null
    const границы = границыМесяца(месяц)
    const расчётныеСчета = данные.счета.map((счёт) => ({
      ...счёт,
      фактическийОстаток: расчётныйОстаток(счёт, данные.операции),
    }))
    const итог = итогПоСчетам(расчётныеСчета)
    const долги = итогОбязательств(данные.обязательства)
    const период = итогПериода(данные.операции, границы.от, границы.до)
    const горизонт = данные.настройки.денежныйГоризонт
    const переносВыходного = горизонт?.переносВыходного ?? 'без переноса'
    const подтверждённыеДоходы = new Set(
      данные.операции
        .filter((операция) => операция.тип === 'доход')
        .map(ключПодтверждённогоДохода)
        .filter((ключ): ключ is string => ключ !== null),
    )
    const следующийДоход = следующееСобытиеДохода({
      сегодня: сегодня(),
      доходы: горизонт?.регулярныеДоходы ?? [],
      переносВыходного,
      подтверждённые: подтверждённыеДоходы,
    })
    const датаДоходаПоСтаромуГрафику = следующаяДатаДохода({
      сегодня: сегодня(),
      дниМесяца: горизонт?.дниДохода ?? [],
      переносВыходного,
    })
    const датаДохода =
      следующийДоход?.дата ??
      датаДоходаПоСтаромуГрафику ??
      горизонт?.датаСледующегоДохода ??
      null
    const операцииIds = new Set(данные.операции.map((операция) => операция.id))
    const платежиДляГоризонта = данные.платежи.map((платёж) =>
      платёж.операцияId !== null && !операцииIds.has(платёж.операцияId)
        ? { ...платёж, операцияId: null }
        : платёж,
    )
    const доДохода = рассчитатьДоДохода({
      сегодня: сегодня(),
      датаСледующегоДохода: датаДохода,
      собственные:
        итог.счетовУчтено > 0 && итог.счетовБезОстатка === 0
          ? итог.собственные
          : null,
      минимальныйРезерв: данные.настройки.резервУжеИсключён
        ? 0
        : данные.настройки.минимальныйРезерв,
      плановыеПлатежи: платежиДляГоризонта,
      обязательства: данные.обязательства,
    })
    const чистаяПозиция = итог.собственные - долги.яДолжен
    ...
    const достаточноИстории = самаяРанняя !== null && самаяРанняя <= границаИстории
  ...
  return (
    <div className="anim-rise space-y-5">
      ...
      <MoneyRunwayCard
        расчёт={посчитано.доДохода}
        настройки={данные.настройки}
        платежи={данные.платежи}
        обязательства={данные.обязательства}
        датаДохода={посчитано.датаДохода}
        следующийДоход={посчитано.следующийДоход}
        счета={данные.счета}
      ...
      <Card>
        <div className="grid grid-cols-2 gap-5 p-5 lg:grid-cols-4">
          ...
          <Metric
            единица="₽ · позиция"
            подпись="После всех долгов"
            счётчик={{ число: посчитано.чистаяПозиция, запись: деньги }}
            источник="собственные − весь известный долг"
            тон={посчитано.чистаяПозиция < 0 ? 'опасность' : 'нейтральный'}
          />
          <Metric
            единица="₽ · долг"
            подпись="Я должен"
        ...
        {посчитано.запас !== null ? (
          <p className="border-t border-line px-5 py-3 text-caption text-ink-3">
            ...
            {склонение(Math.round(посчитано.запас), 'месяц', 'месяца', 'месяцев')}{' '}
            расходов по среднему за последние {МЕСЯЦЕВ_ДЛЯ_ЗАПАСА} месяца.
          </p>
        ) : null}
      </Card>
        ...
        <Card>
          ...
          {посчитано.темп && посчитано.темп.расходыДоСегодня > 0 ? (
            <div className="mx-5 mb-4 space-y-2">
              ...
              {посчитано.доДохода.можноПотратить !== null ? (
                <p
                  className={cn(
                    'rounded-3 px-3 py-2 text-caption',
                    посчитано.темп.прогнозНаОстаток >
                      посчитано.доДохода.можноПотратить
                ...
                >
                  {посчитано.темп.прогнозНаОстаток >
                  посчитано.доДохода.можноПотратить
                    ? `Прогноз расходов до конца месяца больше безопасного остатка до дохода (${деньги(посчитано.доДохода.можноПотратить)}).`
                    : `Безопасного остатка до дохода хватает на прогноз до конца месяца.`}
                </p>
              ) : null}
              <p
                className={cn(
