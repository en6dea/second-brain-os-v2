...
import {
  ...
  type СохранениеРефлексииЦели,
} from './GoalReflectionDialog'
import { применитьРефлексиюЦели } from './model/Reflection'
import { ключПериодаОбзора } from '@/core/reviews/Period'
import { ЗНАЧОК } from '@/design-system/iconSize'
import { GoalPulseCard } from './GoalPulseCard'
...
export function GoalsPage() {
  ...
  const [предложениеХода, установитьПредложениеХода] =
    useState<Partial<ЧерновикСледующегоХода> | null>(null)
  const [рефлексияДляЗадачи, установитьРефлексиюДляЗадачи] =
    useState<РефлексияЦели | null>(null)
  const [рефлексия, установитьРефлексию] = useState<Цель | null>(null)
  const [кУдалению, установитьКУдалению] = useState<Цель | null>(null)
  const [сфераФильтр, установитьСферуФильтр] = useState('')
  const созданиеХодаНачалось = useRef(false)
  ...
  const периодНедели = ключПериодаОбзора('неделя', сегодня())
  ...
  async function сохранить() {
    ...
    if (черновик.id) {
      ...
      if (текущая) {
        const момент = сейчас()
        const новоеТекущее = черновик.текущее ?? null
        const новоеЦелевое = черновик.цель ?? текущая.цель
        const показательИзменился =
          новоеЦелевое !== null &&
          новоеЦелевое > 0 &&
          новоеТекущее !== null &&
          Number.isFinite(новоеТекущее) &&
          новоеТекущее !== текущая.текущее
        await база.goals.put({
          ...текущая,
          ...черновик,
          историяПрогресса: показательИзменился
            ? [
                ...(текущая.историяПрогресса ?? []),
                {
                  id: новыйId(),
                  createdAt: момент,
                  значение: новоеТекущее,
                  заметка: 'Показатель обновлён в карточке цели',
                },
              ]
            : текущая.историяПрогресса,
      ...
      const момент = сейчас()
      const начальноеЗначение = черновик.текущее ?? null
      const целевоеЗначение = черновик.цель ?? null
      await база.goals.add(
        новаяЗапись({
          ...
          историяПрогресса:
            целевоеЗначение !== null &&
            целевоеЗначение > 0 &&
            начальноеЗначение !== null &&
            Number.isFinite(начальноеЗначение)
              ? [
                  {
                    id: новыйId(),
                    createdAt: момент,
                    значение: начальноеЗначение,
                    заметка: 'Начальный показатель цели',
                  },
                ]
              : [],
          рефлексии: [],
  ...
  async function создатьСледующийХод(ход: ЧерновикСледующегоХода) {
    if (!следующийХод || созданиеХодаНачалось.current) return
    созданиеХодаНачалось.current = true
    ...
    try {
      await база.transaction('rw', база.tasks, база.goals, async () => {
        ...
        if (!текущаяЦель) throw new Error('Цель больше не найдена')
        if (рефлексияДляЗадачи) {
          const текущаяРефлексия = (текущаяЦель.рефлексии ?? []).find(
            (запись) => запись.id === рефлексияДляЗадачи.id,
          )
          if (
            !текущаяРефлексия ||
            текущаяРефлексия.updatedAt !== рефлексияДляЗадачи.updatedAt
          ) {
            throw new Error(
              'Рефлексия изменилась в другом окне. Откройте решение недели снова.',
            )
          }
        }
        const новаяЗадача = новаяЗапись({
        ...
        await база.tasks.add(новаяЗадача as never)
        const момент = сейчас()
        await база.goals.put({
          ...текущаяЦель,
          рефлексии: рефлексияДляЗадачи
            ? (текущаяЦель.рефлексии ?? []).map((запись) =>
                запись.id === рефлексияДляЗадачи.id
                  ? {
                      ...запись,
                      задачаId: новаяЗадача.id,
                      updatedAt: момент,
                    }
                  : запись,
              )
            : текущаяЦель.рефлексии,
          последняяАктивность: момент,
          updatedAt: момент,
        })
      })
      установитьСледующийХод(null)
      установитьПредложениеХода(null)
      установитьРефлексиюДляЗадачи(null)
      сообщить('Следующий ход добавлен в задачи')
    } catch (ошибка) {
      созданиеХодаНачалось.current = false
      throw ошибка
    }
  }

  function открытьСледующийХод(цель: Цель, источник: РефлексияЦели | null = null) {
    созданиеХодаНачалось.current = false
    установитьРефлексиюДляЗадачи(источник)
    установитьПредложениеХода(
      источник
        ? {
            название: источник.следующийШаг,
            заметка: источник.вывод
              ? `Вывод недельной рефлексии: ${источник.вывод}`
              : 'Следующий шаг из недельной рефлексии цели',
            длительностьМинут: 25,
            важность: 'обычная',
          }
        : null,
    )
    установитьСледующийХод(цель)
  }

  async function сохранитьРефлексию({
    черновик: черновикРефлексии,
    подготовитьЗадачу,
  }: СохранениеРефлексииЦели) {
    if (!рефлексия) return
    const снимок = рефлексия
    const текущая = await база.goals.get(снимок.id)
    if (!текущая) throw new Error('Цель больше не найдена')
    if (текущая.updatedAt !== снимок.updatedAt) {
      throw new Error(
        'Цель изменилась в другом окне. Закройте форму и откройте снова.',
      )
    }

    const результат = применитьРефлексиюЦели({
      цель: текущая,
      черновик: черновикРефлексии,
      момент: сейчас(),
      рефлексияId: новыйId(),
    ...
    await база.goals.put(результат.цель)
    установитьРефлексию(null)
    сообщить('Рефлексия цели сохранена')

    if (подготовитьЗадачу && результат.рефлексия.следующийШаг) {
      открытьСледующийХод(результат.цель, результат.рефлексия)
    }
  }
  ...
  return (
    <div className="anim-rise space-y-5">
              ...
              <Card key={цель.id}>
                ...
                <div className="space-y-3 px-5 pb-5">
                  ...
                  <GoalPulseCard
                    цель={цель}
                    период={периодНедели}
                    наРефлексию={установитьРефлексию}
                    наПодготовкуЗадачи={открытьСледующийХод}
                  />
                  ...
                  <div className="rounded-3 border border-line bg-sunken px-3 py-3">
                    ...
                    <Button
                      ...
                      размер="малый"
                      className="mt-3"
                      onClick={() => открытьСледующийХод(цель)}
                    >
                      Запланировать ход
                    </Button>
                  </div>
      ...
      <GoalReflectionDialog
        ...
        наЗакрытие={() => установитьРефлексию(null)}
        наПодтверждение={сохранитьРефлексию}
      />

      <GoalNextMoveDialog
        цель={следующийХод}
        предложение={предложениеХода}
        наЗакрытие={() => {
          установитьСледующийХод(null)
          установитьПредложениеХода(null)
          установитьРефлексиюДляЗадачи(null)
