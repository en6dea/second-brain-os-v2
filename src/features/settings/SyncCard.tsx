...
export function SyncCard({ настройки }: { настройки: Настройки }) {
  ...
  // Раньше «вошли» была обычной переменной экрана: сбрасывалась в «нет» при
  // каждом открытии настроек, хотя сам Firebase помнил вход — человека
  // заново просило войти, хотя сессия никуда не девалась. Подписка на
  // наСменуПользователя спрашивает настоящее состояние Firebase, а не
  // хранит своё поверх него — колбэк вызывается сразу с тем, что Firebase
  // уже восстановил из своего хранилища, ещё до первого действия человека.
  useEffect(() => {
    if (!конфигурация) return
    let отменено = false
    let отписаться: (() => void) | undefined

    void (async () => {
      const { убедитьсяИнициализирован, наСменуПользователя } =
        await загрузитьFirebase()
      убедитьсяИнициализирован(конфигурация)
      if (отменено) return
      отписаться = наСменуПользователя((пользователь) => {
        установитьВошли(пользователь !== null)
      })
    })()
